...
function memberDetailHref(memberId, depth = "root") {
  ...
  const prefix =
    depth === "nested" ? "../../members/detail/" : "../members/detail/";
...
function normalizeStatus(value) {
  ...
  if (
    ["success", "ok", "healthy", "done", "active", "completed"].includes(status)
  )
    return "good";
  if (["failed", "error", "critical", "blocked"].includes(status))
    return "danger";
  if (["running", "pending", "warning", "review", "stale"].includes(status))
    return "warn";
  return "";
}
...
const NAV_ICONS = {
  ...
  members:
    "M16 19v-1.5A3.5 3.5 0 0 0 12.5 14h-5A3.5 3.5 0 0 0 4 17.5V19M11 8a3 3 0 1 1-6 0 3 3 0 0 1 6 0M20 19v-1a3 3 0 0 0-3-3h-1.2M15 5.2a2.8 2.8 0 0 1 0 5.6",
  ...
  automation:
    "M12 3v4M12 17v4M4.9 4.9l2.8 2.8M16.3 16.3l2.8 2.8M3 12h4M17 12h4M4.9 19.1l2.8-2.8M16.3 7.7l2.8-2.8M9 12a3 3 0 1 0 6 0 3 3 0 0 0-6 0",
  business: "M4 19h16M6 16V9M12 16V5M18 16v-7",
  imports: "M12 3v11M7 9l5 5 5-5M5 19h14",
  rules: "M6 4h12v16H6zM9 8h6M9 12h6M9 16h4",
  settings:
    "M12 8a4 4 0 1 0 0 8 4 4 0 0 0 0-8M12 3v3M12 18v3M4.6 6.2l2.1 2.1M17.3 15.7l2.1 2.1M3 12h3M18 12h3M4.6 17.8l2.1-2.1M17.3 8.3l2.1-2.1",
};
...
function activateNav() {
  ...
  document.querySelectorAll(".nav a").forEach((link) => {
    ...
    const isRoot =
      ...
      (path.endsWith("/core") || path === "");
    const isActive =
      isRoot || (link.dataset.section && hrefPath && path.endsWith(hrefPath));
...
async function initFirebase() {
  ...
  const [{ initializeApp, getApps }, firestore, auth, functions] =
    await Promise.all([
      import(
        `https://www.gstatic.com/firebasejs/${FIREBASE_APP_VERSION}/firebase-app.js`
      ),
      import(
        `https://www.gstatic.com/firebasejs/${FIREBASE_APP_VERSION}/firebase-firestore.js`
      ),
      import(
        `https://www.gstatic.com/firebasejs/${FIREBASE_APP_VERSION}/firebase-auth.js`
      ),
      import(
        `https://www.gstatic.com/firebasejs/${FIREBASE_APP_VERSION}/firebase-functions.js`
      ),
    ]);
  ...
  state.firebaseRuntime = {
    ...
    functionsClient: functions.getFunctions(
      app,
      config.functionsRegion || "asia-northeast3",
    ),
    auth,
    functions,
    ...firestore,
...
function ensureLoginGate() {
  if (
    qs("coreLoginGate") ||
    !document.querySelector("[data-firestore-dashboard]")
  )
    return;
  ...
  qs("coreLoginForm")?.addEventListener("submit", async (event) => {
    ...
    try {
      ...
      await runtime.auth.signInWithEmailAndPassword(
        runtime.authClient,
        `p${phone}@archivepilates.com`,
        password,
      );
...
function loginErrorMessage(error) {
  ...
  if (code.includes("unauthorized-domain"))
    return "Firebase Auth 허용 도메인 확인이 필요합니다.";
...
async function waitForAuth(runtime) {
  ...
  if (!state.authReady) {
    state.authReady = new Promise((resolve) => {
      const unsubscribe = runtime.auth.onAuthStateChanged(
        runtime.authClient,
        (user) => {
          unsubscribe();
          resolve(user || null);
...
async function callCoreFunction(name, payload) {
  const runtime = await initFirebase();
  const callable = runtime.functions.httpsCallable(
...
async function getRecentCollectionBy(
  db,
  firestore,
  collectionName,
...
async function getCollectionBy(
  db,
  firestore,
  collectionName,
  orderField = "updatedAt",
...
function studioItems(items) {
  return items.filter(
    (item) => String(item.studioId || STUDIO_ID) === STUDIO_ID,
  );
}
...
async function getLimitedSubcollection(
  db,
  firestore,
  memberId,
  subcollection,
  ...
  const snapshot = await firestore.getDocs(
    firestore.query(
      firestore.collection(db, "members", memberId, subcollection),
      firestore.limit(maxItems),
    ),
  );
  return snapshot.docs.map((docSnapshot) => ({
...
async function loadMemberDetail(runtime, memberId) {
  ...
  return {
    ...
    member: memberSnapshot.exists()
      ...
      : null,
    card: cardSnapshot.exists()
      ? { id: cardSnapshot.id, ...cardSnapshot.data() }
      : null,
...
function renderAutomation(items) {
  const failedItems = items.filter((item) =>
    ["failed", "error", "critical", "blocked"].includes(
      String(item.status || item.health || "").toLowerCase(),
    ),
  );
  const latestItem = [...items].sort((a, b) => {
    const left = new Date(
      a.updatedAt?.toDate?.() || a.updatedAt || a.lastRunAt || a.checkedAt || 0,
    ).getTime();
    const right = new Date(
      b.updatedAt?.toDate?.() || b.updatedAt || b.lastRunAt || b.checkedAt || 0,
    ).getTime();
    return right - left;
  })[0];
  const automationMode = qs("automationMode");
  if (automationMode) {
    automationMode.textContent = failedItems.length
      ? "확인 필요"
      : items.length
        ? "연결"
        : "기록 대기";
  ...
  setText(
    ...
    latestItem
      ? formatDate(
          latestItem.updatedAt || latestItem.lastRunAt || latestItem.checkedAt,
        )
      : "기록 대기",
  );
  setText(
    "automationConnectedState",
    items.length ? "상태 문서 연결됨" : "컬렉션 연결 · 기록 없음",
  );
  setText(
  ...
  list.innerHTML = items
    .map((item) => {
      ...
      const updated = formatDate(
        item.updatedAt || item.lastRunAt || item.checkedAt,
      );
      const nextRun =
        item.nextRunAt || item.nextScheduledAt
          ? ` · 다음 ${formatDate(item.nextRunAt || item.nextScheduledAt)}`
          : "";
      const error =
    ...
    const flows = [
      {
        ...
        source:
          items.find((item) => item.id === "studiomate-excel-sync") || items[0],
        detail:
          "회원목록, 예약내역, 삭제 수업 원본을 가져와 CORE 표시용 상태를 갱신합니다.",
      },
      {
        title: "알림톡 자동발송",
        source: items.find((item) =>
          String(item.id || "").includes("alimtalk"),
        ),
        detail:
          "후보 선정과 실제 발송은 기존 canonical 컬렉션 기준으로 유지합니다.",
      },
      {
    ...
    healthList.innerHTML = flows
      .map((flow) => {
        const status =
          flow.source?.status ||
          flow.source?.health ||
          (flow.source ? "active" : "pending");
        const updated = flow.source
          ? formatDate(
              flow.source.updatedAt ||
                flow.source.lastRunAt ||
                flow.source.checkedAt,
            )
          : "기록 대기";
        const lastResult =
          flow.source?.lastResult ||
          flow.source?.summary ||
          flow.source?.message ||
          flow.detail;
        return `
...
function renderImports(items) {
  ...
  setText(
    "importConnectionState",
    items.length ? "최근 원본 처리 기록 연결됨" : "sourceImports 기록 대기",
  );
  ...
  setText(
    ...
    latest
      ? sourceKindLabel(
          latest.sourceKind || latest.kind || latest.fileName || latest.id,
        )
      : "원본 대기",
  );
  setText(
  ...
  table.innerHTML = items
    .map((item) => {
      ...
      const updated = formatDate(
        item.updatedAt || item.importedAt || item.createdAt,
      );
      return `
        <tr>
...
function renderQualityIssues(items) {
  const activeIssues = items.filter(
    (item) =>
      !["resolved", "closed", "done"].includes(
        String(item.status || "").toLowerCase(),
      ),
  );
  const resolvedIssues = items.filter((item) =>
    ["resolved", "closed", "done"].includes(
      String(item.status || "").toLowerCase(),
    ),
  );
  ...
  setText(
    "latestQualityTitle",
    latestIssue
      ? latestIssue.title || latestIssue.issueType || "품질 이슈"
      : "열린 이슈 없음",
  );
  setText(
    "latestQualityNote",
  ...
  list.innerHTML = activeIssues
    .map((item) => {
      ...
      const detail =
        item.summary || item.description || item.memberName || "상세 기록 없음";
      const action = qualityActionText(item);
      const href = item.memberId ? memberDetailHref(item.memberId) : null;
      const tagName = href ? "a" : "div";
      const attrs = href
        ? ` class="status-row status-link" href="${escapeHtml(href)}"`
        : ` class="status-row"`;
      return `
        <${tagName}${attrs}>
          <div>
            <strong>${escapeHtml(title)}</strong>
            <p>${escapeHtml(detail)} · ${escapeHtml(action)} · ${escapeHtml(
              (item.sourcePaths || [])
...
function qualityActionText(item) {
  const type = String(
    item.issueType || item.type || item.title || "",
  ).toLowerCase();
  if (type.includes("phone") || type.includes("전화"))
    return "전화번호 확인 전 외부 실행 보류";
  if (type.includes("duplicate") || type.includes("중복"))
    return "canonical key 기준 우선순위 확인";
  if (type.includes("name") || type.includes("동명이인"))
    return "이름 단독 매칭 금지, 전화번호/StudioMate ID 확인";
  if (type.includes("excel")) return "실제 StudioMate memberId 해소 후 사용";
  return "운영자가 원천/매칭 상태 확인";
}

function activeQualityIssues() {
  return state.qualityIssues.filter(
    (item) =>
      !["resolved", "closed", "done"].includes(
        String(item.status || "").toLowerCase(),
      ),
...
function memberHasQualityIssue(item) {
  ...
  return activeIssues.some((issue) => {
    const target = [
      ...
      .join(" ");
    return (
      (memberId && target.includes(memberId)) ||
      (memberName && target.includes(memberName))
    );
  });
}
...
function matchesMemberSearch(item) {
  ...
  const haystack = [
    ...
    item.phoneLast4,
    ...(item.tagLabels || []),
    ...(item.currentTicketsSummary || []).map((ticket) =>
      typeof ticket === "string" ? ticket : ticket.name || ticket.ticketName,
    ),
  ]
    .filter(Boolean)
    .join(" ")
...
function matchesMemberFilter(item) {
  if (memberFilter === "active-ticket")
    return toNumber(item.activeTicketCount) > 0;
...
function renderMemberFilterButtons(items) {
  const counts = {
    all: items.length,
    "active-ticket": items.filter(
      (item) => toNumber(item.activeTicketCount) > 0,
    ).length,
...
function renderMembers(items) {
  ...
  setText(
    "membersActiveTicketCount",
    String(
      visibleItems.filter((item) => toNumber(item.activeTicketCount) > 0)
        .length,
    ),
  );
  setText(
    "membersRecentVisitCount",
    String(visibleItems.filter((item) => item.recentVisitAt).length),
  );
  setText(
    "membersRevenueCount",
    String(
      visibleItems.filter((item) => toNumber(item.totalRevenue) > 0).length,
    ),
  );
  ...
  table.innerHTML = visibleItems
    .map((item) => {
      const ticketNames = (
        item.currentTicketsSummary ||
        item.activeTicketNames ||
        []
      )
        .map((ticket) => (typeof ticket === "string" ? ticket : ticket.name))
        .filter(Boolean)
        .slice(0, 2);
      const ticketText = ticketNames.length
        ? ticketNames.join(", ")
        : "활성 수강권 없음";
...
function renderMessages(candidates, sends) {
  ...
  setText(
    "messagesPendingDecision",
    pendingCandidates.length
      ? `${pendingCandidates.length}건 확인`
      : "대기 없음",
  );
  setText(
    "messagesRiskDecision",
    failedSends.length ? `${failedSends.length}건 실패` : "위험 낮음",
  );
  ...
  const renderAlimtalkRow = (item, options = {}) => {
    ...
    const date = formatDate(
      item.sentAt || item.updatedAt || item.createdAt || item.sourceDate,
    );
    const reason =
      item.reason ||
      item.lastError ||
  ...
  if (sendList) {
    sendList.innerHTML = sends.length
      ? sends
          .map((item) =>
            renderAlimtalkRow(item, {
              status: (send) => send.status || "done",
            }),
          )
          .join("")
  ...
  if (templateList) {
    ...
    for (const item of [...candidates, ...sends]) {
      ...
      if (["sent", "done", "success", "completed"].includes(status))
        current.sent += 1;
    ...
    const rows = [...templateMap.values()]
      .sort(
        (a, b) =>
          b.failed - a.failed ||
          b.sends + b.candidates - (a.sends + a.candidates),
      )
      .slice(0, 8);
  ...
  if (decisionList) {
    const rows = [
      ...
      },
      {
        title: "운영 경계",
        detail:
          "이 탭은 확인용입니다. 후보 선정과 발송 원천은 기존 alimtalkCandidates/alimtalkSends를 유지합니다.",
        status: "active",
      },
    ];
...
function renderMemberDetail(detail) {
  ...
  if (detail?.missingId) {
    ...
    setText(
      ...
      "Members 목록에서 회원을 선택하면 상세 read-model을 표시합니다.",
    );
    return;
  }
  if (detail?.missing) {
  ...
  const tickets = detail?.tickets?.length
    ? detail.tickets
    : merged.currentTicketsSummary || [];
  const purchases = detail?.purchases?.length
    ? detail.purchases
    : merged.recentPurchases || [];
  const bookings = detail?.bookings?.length
    ? detail.bookings
    : merged.recentBookings || [];
  const memos = detail?.memos?.length ? detail.memos : merged.recentMemos || [];
  const alimtalkLogs = detail?.alimtalkLogs?.length
    ? detail.alimtalkLogs
    : merged.recentAlimtalk || [];
  const tags = detail?.tags?.length ? detail.tags : merged.tags || [];
  const relatedIssues = state.qualityIssues.filter((item) => {
    const target = [item.memberId, item.memberName, item.name, item.profileId]
      .filter(Boolean)
      .join(" ");
    return (
      target.includes(member.memberId || detail?.id || "") ||
      target.includes(member.name || member.memberName || "")
    );
  });
  const openSignals = signals.filter((signal) => {
    const level =
      typeof signal === "string" ? "" : signal.level || signal.severity;
    return ["critical", "danger", "error", "warning", "warn"].includes(
  ...
  setText(
    "memberDetailName",
    member.name || member.memberName || detail?.id || "회원",
  );
  setText(
    "memberDetailSubtitle",
  ...
  setText(
    "memberDetailTickets",
    formatCount(member.activeTicketCount || tickets.length, "개"),
  );
  setText(
    "memberDetailBookings",
    formatCount(member.bookingCount || bookings.length),
  );
  ...
  setText(
    "memberDetailUpdatedAt",
    formatDate(member.updatedAt || summary.updatedAt),
  );
  ...
  const activeTicketCount = toNumber(
    member.activeTicketCount || tickets.length,
  );
  const lastVisitText = member.recentVisitAt
    ? compactDateTime(member.recentVisitAt)
    : "최근 방문 없음";
  if (relatedIssues.length) {
    setPillText("memberDetailPrimaryActionTone", "warning");
    setText("memberDetailPrimaryAction", "품질 이슈 먼저 확인");
    setText(
      "memberDetailPrimaryActionNote",
      "전화번호, 임시 ID, 중복 fallback 여부를 확인하기 전에는 외부 실행으로 넘기지 않습니다.",
    );
    ...
    setText(
      "memberDetailPrimaryActionNote",
      "수강권, 메모, 알림톡 상태를 먼저 확인하고 필요하면 운영자가 직접 판단합니다.",
    );
  } else {
    ...
    setText(
      "memberDetailPrimaryActionNote",
      "현재 read-model 기준으로 즉시 멈춰야 할 품질/주의 신호는 보이지 않습니다.",
    );
  }
  setPillText(
    "memberDetailCareActionTone",
    activeTicketCount ? "active" : "warning",
  );
  setText(
    "memberDetailCareAction",
    activeTicketCount ? "수강권 기반 케어" : "수강권 상태 확인",
  );
  setText(
  ...
  setText(
    "memberDetailGuardrailNote",
    "알림톡, 연락처, StudioMate write는 canonical source에서만 대상자를 선정합니다.",
  );
  ...
  if (decisionList) {
    const rows = [
      ...
      {
        title: toNumber(member.activeTicketCount || tickets.length)
          ? "수강권 보유"
          : "수강권 확인 필요",
        ...
        status: toNumber(member.activeTicketCount || tickets.length)
          ? "active"
          : "warning",
      },
      {
        title: relatedIssues.length
          ? "데이터 품질 이슈 있음"
          : "품질 이슈 없음",
  ...
  if (nextList) {
    ...
    const links = [
      {
        title: "알림톡 이력 확인",
        detail: "최근 후보/발송 로그와 템플릿 상태를 함께 봅니다.",
        href: "../../messages/",
        status: "active",
      },
      {
        title: "원본 품질 확인",
        detail: "중복, 임시 ID, name-only match 여부를 확인합니다.",
        href: "../../imports/",
        status: relatedIssues.length ? "warning" : "active",
      },
      {
        title: "경영 맥락 확인",
        detail: "매출 상위/ACM 후보/장기회원 판단 흐름과 연결합니다.",
        href: "../../business/",
        status: toNumber(member.totalRevenue) ? "success" : "active",
      },
      {
        title: "회원 목록으로 돌아가기",
        detail: id
  ...
  renderMiniList("memberDetailPurchasesList", purchases, {
    ...
    title: (item) =>
      item.ticketName || item.productName || item.name || item.id,
    detail: (item) =>
      [
        shortDate(item.purchasedAt || item.createdAt),
        formatManwon(toNumber(item.price || item.amount || item.revenue)),
      ]
        .filter(Boolean)
        .join(" · "),
  ...
  renderMiniList("memberDetailBookingsList", bookings, {
    ...
    title: (item) =>
      item.lessonTitle || item.lectureTitle || item.ticketName || item.id,
    detail: (item) =>
      [
        shortDate(item.lessonDate || item.startsAt || item.startAt),
        item.startTime,
        item.staffName || item.instructorName,
      ]
        .filter(Boolean)
        .join(" · "),
  ...
  renderMiniList("memberDetailMemosList", memos, {
    ...
    detail: (item) =>
      item.memo || item.content || item.text || shortDate(item.createdAt),
    status: (item) => item.syncStatus || item.status,
  });
  renderMiniList("memberDetailAlimtalkList", alimtalkLogs, {
    empty: "최근 알림톡 기록이 없습니다.",
    title: (item) =>
      item.templateName || item.templateCode || item.candidateType || item.id,
    detail: (item) =>
      [
        shortDate(item.sentAt || item.createdAt),
        item.reason || item.message || item.managementNumber,
      ]
        .filter(Boolean)
        .join(" · "),
  ...
  if (tagList) {
    const values = tags
      .map((tag) =>
        typeof tag === "string"
          ? tag
          : tag.label || tag.name || tag.tag || tag.id,
      )
      .filter(Boolean)
      .slice(0, 24);
  ...
  renderMiniList("memberDetailQualityList", relatedIssues, {
    ...
    detail: (item) =>
      [
        item.summary || item.description || "상세 기록 없음",
        qualityActionText(item),
      ]
        .filter(Boolean)
        .join(" · "),
...
function renderPrivate(requests, records, usageEvents, ledgerEntries) {
  ...
  const recordFailures = records.filter((item) =>
    ["failed", "error"].includes(
      String(item.gptStatus || item.status || "").toLowerCase(),
    ),
  ).length;
  const corrections = records.filter(
    (item) => item.sessionNumberCorrection,
  ).length;
  const submitted = requests.filter((item) =>
    ["submitted", "pre_submitted", "post_submitted", "completed"].includes(
      String(
        item.preStatus || item.postStatus || item.status || "",
      ).toLowerCase(),
    ),
  ).length;
  setText(
    "privateRecordHealth",
    recordFailures ? `${recordFailures}건 오류 확인` : "차트 기록 읽기 정상",
  );
      ...
      ? requests
          .map((item) => {
            const status =
              ...
              item.postStatus ||
              item.status ||
              item.alimtalk?.status ||
              "pending";
            const lesson = [item.lessonDate, item.staffName]
              .filter(Boolean)
              .join(" · ");
            return `
              <div class="status-row">
  ...
  container.innerHTML = rows
    .map((row) => {
      const percent = Math.max(
        6,
        Math.round((row.totalRevenue / maxRevenue) * 100),
      );
...
function renderBusinessRanks(snapshot, month) {
  ...
  const instructors = snapshot.instructorRevenue
    ...
    .slice(0, 4);
  const tickets = snapshot.ticketTop
    .filter((row) => row.month === month)
    .slice(0, 4);
  ...
  container.innerHTML = [
    instructors.length
      ? renderRows("강사 수업매출", instructors, "revenue")
      : "",
...
function renderBusinessMonth(month) {
  ...
  if (!snapshot) return;
  const current =
    snapshot.summary.find((row) => row.month === month) ||
    snapshot.summary.at(-1);
  if (!current) return;
  const previousIndex =
    snapshot.summary.findIndex((row) => row.month === current.month) - 1;
  ...
  setText(
    "businessHeroNote",
    daily
      ? `${daily.date} 누적 기준 · 기존 현황판 원천`
      : "월 summary 기준 · 기존 현황판 원천",
  );
  setText(
    "businessTotalRevenue",
    formatManwon(daily?.totalRevenue || current.totalRevenue),
  );
  setText(
    "businessLessonRevenue",
    formatManwon(daily?.lessonRevenue || current.lessonRevenue),
  );
  setText(
    "businessMarginRate",
    formatRate(daily?.marginRate || current.marginRate),
  );
  setText(
    "businessAttendanceRate",
    formatRate(daily?.attendanceRate || current.attendanceRate),
  );
  setText(
    "businessTotalDelta",
    daily?.previousTotalRevenue
      ? deltaText(daily.totalRevenue, daily.previousTotalRevenue)
      : deltaText(current.totalRevenue, previous?.totalRevenue),
  );
  setText(
    "businessLessonDelta",
    daily?.previousLessonRevenue
      ? deltaText(daily.lessonRevenue, daily.previousLessonRevenue)
      : deltaText(current.lessonRevenue, previous?.lessonRevenue),
  );
  setText(
    "businessMarginDelta",
    daily?.previousMarginRate
      ? deltaText(daily.marginRate, daily.previousMarginRate, "%p")
      : deltaText(current.marginRate, previous?.marginRate, "%p"),
  );
  setText(
...
function renderBusiness(snapshot) {
  ...
  setText(
    "businessUpdatedAt",
    snapshot.updatedAt ? formatDate(snapshot.updatedAt) : "업데이트 확인",
  );
...
function renderBusinessMemberInsights(items) {
  ...
  setText(
    ...
    items.length ? `${items.length}명 read-model` : "회원 지표 대기",
  );
  if (!items.length) {
  ...
  list.innerHTML = items
    .map((item, index) => {
      ...
      const visits = item.attendedCount
        ? `출석 ${item.attendedCount}회`
        : "출석 요약 대기";
      const tickets = item.activeTicketCount
        ? `활성 ${item.activeTicketCount}개`
        : "활성 수강권 없음";
      return `
...
function renderHomeDecisions() {
  ...
  const rows = [
    ...
    },
    {
      ...
      detail:
        "ARCHIVE CORE는 현재 운영 판단 콘솔입니다. 외부 발송/쓰기 원천은 기존 canonical 컬렉션을 유지합니다.",
      status: "active",
      href: "./rules/",
    },
  ];
...
function renderHomeSummary() {
  ...
  setText(
    "homeMemberNote",
    `활성 수강권 ${activeMembers.toLocaleString("ko-KR")}명 · 전체 회원 검색 가능`,
  );
  ...
  setText(
    "homeAutomationTotal",
    failedAutomation.length ? `${failedAutomation.length}건 확인` : "정상권",
  );
  setText(
    "homeAutomationNote",
    openIssues.length
      ? `품질 이슈 ${openIssues.length}건과 함께 확인`
      : "자동화 실패/품질 위험 낮음",
  );
}
...
function renderBusinessFallback(error) {
  ...
  setText(
    "businessHeroNote",
    error?.message || "Firestore 권한 또는 snapshot 문서 확인이 필요합니다.",
  );
...
function setCheckinState(label, tone = "") {
  const element = qs("checkinStateLabel");
  if (!element) return;
  element.textContent = label;
  element.className = `checkin-state ${tone}`;
}

function setCheckinMessage(message, tone = "") {
  const elements = [qs("checkinMessage"), qs("checkinEntryMessage")].filter(
    Boolean,
  );
  for (const element of elements) {
    element.textContent = message;
    element.className = `checkin-message ${tone}`;
  }
}

function setCheckinStage(stage) {
  const shell = document.querySelector("[data-checkin-stage]");
  if (!shell) return;
  shell.setAttribute(
    "data-checkin-stage",
    stage === "confirm" ? "confirm" : "entry",
  );
}

function updateCheckinClock() {
  if (!isCheckinApp()) return;
  const now = new Date();
  setText(
    "checkinDateLabel",
...
function renderCheckinDigits() {
  const digits = state.checkin.phoneLast4;
  document.querySelectorAll("[data-digit-dot]").forEach((dot, index) => {
    dot.classList.toggle("is-filled", index < digits.length);
    dot.textContent = index < digits.length ? digits[index] : "";
  });
  const input = qs("checkinPhoneInput");
  if (input) input.value = digits;
}

function resetCheckin() {
  if (state.checkin.parkingPollTimer) {
    window.clearTimeout(state.checkin.parkingPollTimer);
    state.checkin.parkingPollTimer = null;
  }
  state.checkin = {
    phoneLast4: "",
    matches: [],
  ...
  setCheckinStage("entry");
  renderCheckinDigits();
  setCheckinState("입력 대기", "good");
  setCheckinMessage(
    "번호를 입력하면 오늘 예약된 수업과 등록 차량을 확인합니다.",
  );
  const matches = qs("checkinMatches");
  if (matches) matches.innerHTML = "";
}

function appendCheckinDigit(digit) {
  if (!/^\d$/.test(digit) || state.checkin.phoneLast4.length >= 4) return;
  state.checkin.phoneLast4 += digit;
  renderCheckinDigits();
  if (state.checkin.phoneLast4.length === 4) lookupCheckin();
}

function backspaceCheckinDigit() {
  state.checkin.phoneLast4 = state.checkin.phoneLast4.slice(0, -1);
  renderCheckinDigits();
}

async function lookupCheckin() {
  const phoneLast4 = state.checkin.phoneLast4;
  if (!/^\d{4}$/.test(phoneLast4)) {
    setCheckinMessage("휴대폰번호 뒤 4자리를 입력해 주세요.", "warn");
    return;
  }
  setCheckinState("조회 중", "warn");
  setCheckinMessage("오늘 예약을 확인하고 있습니다.");
  try {
    const ready = await prepareCheckinDevice();
    if (!ready) return;
    const result = await callCoreFunction("lookupKioskCheckin", {
      ...checkinDevicePayload(),
      phoneLast4,
    });
    state.checkin.matches = result.matches || [];
    state.checkin.selectedMember = state.checkin.matches[0] || null;
    state.checkin.selectedBooking =
      state.checkin.selectedMember?.bookings?.[0] || null;
    state.checkin.selectedVehicle =
      state.checkin.selectedMember?.vehicles?.find(
        (vehicle) => vehicle.isDefault,
      ) ||
      state.checkin.selectedMember?.vehicles?.[0] ||
      null;
    if (!state.checkin.matches.length) {
      setCheckinState("예약 없음", "warn");
      setCheckinMessage(
        "오늘 예약된 수업을 찾지 못했습니다. 휴대폰번호 끝자리를 다시 확인해 주세요.",
        "warn",
      );
    } else {
      setCheckinStage("confirm");
      setCheckinState(`${state.checkin.matches.length}명 확인`, "good");
      setCheckinMessage(
        "회원명과 수업, 차량번호를 확인한 뒤 출석체크를 눌러 주세요.",
        "good",
      );
    }
...
function renderCheckinMatches() {
  const container = qs("checkinMatches");
  if (!container) return;
  const matches = state.checkin.matches;
  if (!matches.length) {
    container.innerHTML = "";
    return;
  }
  container.innerHTML = matches.map(renderCheckinMemberCard).join("");
}

function renderCheckinMemberCard(member) {
  const selected = state.checkin.selectedMember?.memberId === member.memberId;
  ...
  const selectedBooking = selected
    ...
    : member.bookings?.[0];
  return `
    <section class="checkin-member ${selected ? "is-selected" : ""}" data-checkin-member-id="${escapeHtml(member.memberId)}">
      <div class="checkin-member-top">
        <div>
          <strong>${escapeHtml(member.memberName)}</strong>
          <span>휴대폰 끝자리 ${escapeHtml(member.phoneLast4)}</span>
        </div>
        ${member.bookings.length > 1 ? `<em>${member.bookings.length}개 예약</em>` : `<em>오늘 예약</em>`}
      </div>

      <div class="checkin-bookings">
        ${member.bookings
          .map(
            (booking) => `
              <button class="checkin-booking ${selectedBooking?.bookingId === booking.bookingId ? "is-selected" : ""}" type="button" data-checkin-select-booking="${escapeHtml(booking.bookingId)}">
                <span>${escapeHtml(booking.startAtText)}-${escapeHtml(booking.endAtText)}</span>
                <strong>${escapeHtml(booking.lessonTitle || booking.ticketName || "수업")}</strong>
                <small>${escapeHtml(booking.staffName)} · ${escapeHtml(checkinAttendanceLabel(booking.attendanceStatus))}</small>
              </button>
            `,
          )
          .join("")}
      ...
      <div class="checkin-vehicle-box">
        <div class="checkin-vehicle-title">
          <strong>차량번호</strong>
          <span>등록 차량이 있으면 바로 주차등록까지 진행합니다.</span>
        </div>
        <div class="checkin-vehicles">
          ${vehicles
            .map(
              (vehicle) => `
                <button class="checkin-vehicle ${selectedVehicle?.vehicleId === vehicle.vehicleId ? "is-selected" : ""}" type="button" data-checkin-select-vehicle="${escapeHtml(vehicle.vehicleId)}">
                  ${escapeHtml(vehicle.label || vehicle.carNumber)}
                </button>
              `,
            )
            .join("")}
        </div>
        <label class="checkin-car-field">
          <span>${vehicles.length ? "새 차량으로 변경" : "차량번호 등록"}</span>
          <input type="text" data-checkin-car-input placeholder="예: 241고2299" autocomplete="off" />
        </label>
      </div>

      <div class="checkin-actions">
        <button class="checkin-submit primary" type="button" data-checkin-submit="parking">
          출석완료 + 주차등록
        </button>
        <button class="checkin-submit secondary" type="button" data-checkin-submit="attendance-only">
          차량 없이 출석완료
        </button>
      </div>
    </section>
...
function selectCheckinBooking(bookingId) {
  for (const member of state.checkin.matches) {
    const booking = member.bookings.find(
      (item) => item.bookingId === bookingId,
    );
    if (!booking) continue;
    state.checkin.selectedMember = member;
    state.checkin.selectedBooking = booking;
    state.checkin.selectedVehicle =
      member.vehicles.find(
        (vehicle) =>
          vehicle.vehicleId === state.checkin.selectedVehicle?.vehicleId,
      ) ||
      member.vehicles.find((vehicle) => vehicle.isDefault) ||
      member.vehicles[0] ||
      null;
    renderCheckinMatches();
    return;
...
function selectCheckinVehicle(memberId, vehicleId) {
  const member = state.checkin.matches.find(
    (item) => item.memberId === memberId,
  );
  if (!member) return;
  state.checkin.selectedMember = member;
  state.checkin.selectedBooking =
    state.checkin.selectedBooking || member.bookings[0] || null;
  state.checkin.selectedVehicle =
    member.vehicles.find((vehicle) => vehicle.vehicleId === vehicleId) || null;
  renderCheckinMatches();
}

async function submitCheckin(button) {
  const card = button.closest("[data-checkin-member-id]");
  const memberId =
    card?.getAttribute("data-checkin-member-id") ||
    state.checkin.selectedMember?.memberId ||
    "";
  const member =
    state.checkin.matches.find((item) => item.memberId === memberId) ||
    state.checkin.selectedMember;
  const booking =
    state.checkin.selectedMember?.memberId === memberId
      ? state.checkin.selectedBooking
      : member?.bookings?.[0];
  if (!member || !booking) {
    setCheckinMessage("출석 처리할 예약을 선택해 주세요.", "warn");
    return;
  }
  const mode = button.getAttribute("data-checkin-submit");
  const typedCar =
    card?.querySelector("[data-checkin-car-input]")?.value?.trim() || "";
  const savedCar =
    state.checkin.selectedMember?.memberId === memberId
      ? state.checkin.selectedVehicle?.carNumber || ""
      : "";
  const carNumber = typedCar || savedCar;
  const applyParking = mode !== "attendance-only" && Boolean(carNumber);
  if (mode !== "attendance-only" && !carNumber) {
    setCheckinMessage(
      "주차등록을 하려면 차량번호를 입력하거나 등록 차량을 선택해 주세요.",
      "warn",
    );
    return;
  }
  ...
  setCheckinState("처리 중", "warn");
  setCheckinMessage(
    applyParking
      ? "출석 처리와 주차등록을 진행하고 있습니다."
      : "출석 처리를 진행하고 있습니다.",
  );

  try {
    const result = await callCoreFunction("submitKioskCheckin", {
      ...checkinDevicePayload(),
      bookingId: booking.bookingId,
      memberId: member.memberId,
      carNumber,
      saveVehicle: Boolean(carNumber),
      applyParking,
    });
    booking.attendanceStatus = "attended";
    booking.alreadyCheckedIn = true;
    booking.parkingDiscountJobId = result.parkingDiscountJobId || "";
    setCheckinState("출석 완료", "good");
    if (result.parkingDiscountJobId) {
      setCheckinMessage("출석완료. 주차등록 결과를 확인하고 있습니다.", "good");
      await watchParkingJob(result.parkingDiscountJobId);
    } else {
      setCheckinMessage(
        "출석완료. 차량 주차등록은 요청하지 않았습니다.",
        "good",
      );
      setTimeout(resetCheckin, 3600);
    }
    renderCheckinMatches();
  } catch (error) {
    setCheckinState("실패", "danger");
    setCheckinMessage(
      error?.message || "출석 처리에 실패했습니다. 직원을 호출해 주세요.",
      "danger",
    );
  } finally {
    document.querySelectorAll(".checkin-submit").forEach((submitButton) => {
...
async function watchParkingJob(jobId) {
  if (state.checkin.parkingPollTimer) {
    window.clearTimeout(state.checkin.parkingPollTimer);
    state.checkin.parkingPollTimer = null;
  }

  const poll = async (remainingAttempts) => {
    try {
      const job = await callCoreFunction("getKioskParkingJobStatus", {
        ...checkinDevicePayload(),
        jobId,
      });
      const status = String(job.status || "pending");
      if (status === "success") {
        setCheckinState("주차등록 완료", "good");
        setCheckinMessage(
          job.reason === "already_applied"
            ? "출석완료. 이미 2시간 주차할인이 적용되어 있습니다."
            : "출석완료. 2시간 주차할인이 적용되었습니다.",
          "good",
        );
        window.setTimeout(resetCheckin, 4200);
        return;
      }
      if (status === "manual_review") {
        setCheckinState("직원 확인", "warn");
        setCheckinMessage(
          job.lastError ||
            "출석은 완료됐고, 주차등록은 직원 확인이 필요합니다.",
          "warn",
        );
        return;
      }
      if (status === "error") {
        setCheckinState("주차 실패", "danger");
        setCheckinMessage(
          job.lastError ||
            "출석은 완료됐고, 주차등록은 실패했습니다. 직원을 호출해 주세요.",
          "danger",
        );
        return;
      }
      if (status === "eligible") {
        setCheckinState("주차 확인", "warn");
        setCheckinMessage("출석완료. 주차등록 dry-run 상태입니다.", "warn");
        return;
      }
      if (remainingAttempts <= 0) {
        setCheckinState("주차 확인 중", "warn");
        setCheckinMessage(
          "출석은 완료됐고, 주차등록 결과 확인이 지연되고 있습니다. 잠시 후 직원이 확인합니다.",
          "warn",
        );
        return;
      }
      setCheckinState("주차 진행 중", "warn");
      setCheckinMessage("출석완료. 주차등록을 진행하고 있습니다.");
      state.checkin.parkingPollTimer = window.setTimeout(
        () => poll(remainingAttempts - 1),
        1500,
      );
    } catch (error) {
      setCheckinState("상태 확인 실패", "danger");
      setCheckinMessage(
        error?.message || "주차등록 상태를 읽지 못했습니다.",
        "danger",
      );
    }
  };
...
function checkinAttendanceLabel(status) {
  if (status === "attended") return "출석완료";
  if (status === "absent") return "결석";
  if (status === "late_cancel") return "당일취소";
  return "예약확정";
}
...
async function refresh() {
  ...
  try {
    ...
    const [
      ...
      shouldLoadBusiness
        ? getDoc(doc(db, "dashboardSnapshots", "current"))
        : Promise.resolve(null),
      shouldLoadMembers || shouldLoadHome
        ? getCollectionBy(db, runtime, "member360Cards", "totalRevenue", 2000)
        : Promise.resolve([]),
      shouldLoadMessages || shouldLoadHome
        ? getRecentCollectionBy(
            db,
            runtime,
            "alimtalkCandidates",
            "updatedAt",
        ...
        : Promise.resolve([]),
      shouldLoadMessages || shouldLoadHome
        ? getRecentCollectionBy(db, runtime, "alimtalkSends", "updatedAt", 12)
        : Promise.resolve([]),
      shouldLoadMemberDetail
        ? loadMemberDetail(runtime, memberDetailId())
        : Promise.resolve(null),
      shouldLoadBusiness
        ? getRecentCollectionBy(
            db,
    ...
    state.lane = laneSnapshot.exists()
      ? laneSnapshot.data()
      : { status: "active" };
    ...
    if (shouldLoadBusiness) {
      if (dashboardSnapshot?.exists())
        renderBusiness(normalizeBusinessSnapshot(dashboardSnapshot.data()));
      else
        renderBusinessFallback(
          new Error("dashboardSnapshots/current 문서가 없습니다."),
        );
...
qs("businessMonthSelect")?.addEventListener("change", (event) =>
  renderBusinessMonth(event.target.value),
);
...
document.addEventListener("click", (event) => {
  const key = event.target.closest("[data-checkin-key]");
  if (key) {
    appendCheckinDigit(key.getAttribute("data-checkin-key") || "");
    return;
  }
  const action = event.target.closest("[data-checkin-action]");
  if (action?.getAttribute("data-checkin-action") === "backspace") {
    backspaceCheckinDigit();
    return;
  }
  if (action?.getAttribute("data-checkin-action") === "lookup") {
    lookupCheckin();
    return;
  }
  const bookingButton = event.target.closest("[data-checkin-select-booking]");
  if (bookingButton) {
    selectCheckinBooking(
      bookingButton.getAttribute("data-checkin-select-booking") || "",
    );
    return;
  }
  const vehicleButton = event.target.closest("[data-checkin-select-vehicle]");
  if (vehicleButton) {
    const card = vehicleButton.closest("[data-checkin-member-id]");
    selectCheckinVehicle(
      card?.getAttribute("data-checkin-member-id") || "",
      vehicleButton.getAttribute("data-checkin-select-vehicle") || "",
    );
    return;
  }
  const submitButton = event.target.closest("[data-checkin-submit]");
  if (submitButton) {
    submitCheckin(submitButton);
  }
});
qs("checkinResetButton")?.addEventListener("click", resetCheckin);
qs("checkinResultResetButton")?.addEventListener("click", resetCheckin);
qs("checkinPhoneInput")?.addEventListener("input", (event) => {
  state.checkin.phoneLast4 = event.target.value.replace(/\D/g, "").slice(0, 4);
  renderCheckinDigits();
});
if (document.querySelector("[data-firestore-dashboard]")) refresh();
if (isCheckinApp()) {
  updateCheckinClock();
  setInterval(updateCheckinClock, 1000 * 15);
  prepareCheckinDevice();
}
