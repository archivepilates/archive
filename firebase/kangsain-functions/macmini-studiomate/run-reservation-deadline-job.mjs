...
try {
  ...
  reportSubject = `[예약가능기한][성공] 설정 갱신 · ${date}`;
  reportBody = successBody(latest);
  reportLabel = "자동화 성공";
} catch (error) {
  ...
  if (recovery.ok) {
    ...
    reportSubject = `[예약가능기한][성공] 재시도 복구 · ${date}`;
    reportBody = recoveryBody(error.message, recovery.latest);
    reportLabel = "자동화 성공";
  } else {
    result.ok = false;
    result.error = recovery.error || error.message;
    reportSubject = `[예약가능기한][실패] 설정 실패 · ${date}`;
    reportBody = failureBody(result.error);
    reportLabel = "자동화 실패";
    process.exitCode = 1;
  }
}
...
try {
  await sendReport(reportSubject, reportBody, reportLabel);
  result.reportOk = true;
} catch (reportError) {
  result.reportOk = false;
  result.reportError = reportError.message;
  process.exitCode = 1;
}
...
console.log(JSON.stringify({
  ...
  error: result.error || result.reportError || null,
}, null, 2));
