...
function normalizeDailyRevenueRow(row: SheetRow): DashboardDailyRevenueRow {
  return {
    ...
    월누적세전총액: numberValue(firstValue(row, ["월누적세전총액", "mtdPretaxPay"])),
    전월동일일세전총액: numberValue(firstValue(row, ["전월동일일세전총액", "prevMonthSameDayPretaxPay"])),
    월누적수업마진률: numberValue(firstValue(row, ["월누적수업마진률", "mtdLessonMarginRate"])),
    전월동일일수업마진률: numberValue(firstValue(row, ["전월동일일수업마진률", "prevMonthSameDayLessonMarginRate"])),
    월누적그룹세션: numberValue(firstValue(row, ["월누적그룹세션", "mtdGroupSessions"])),
    전월동일일그룹세션: numberValue(firstValue(row, ["전월동일일그룹세션", "prevMonthSameDayGroupSessions"])),
    월누적프라이빗: numberValue(firstValue(row, ["월누적프라이빗", "mtdPrivateSessions"])),
    전월동일일프라이빗: numberValue(firstValue(row, ["전월동일일프라이빗", "prevMonthSameDayPrivateSessions"])),
    월누적강사레슨: numberValue(firstValue(row, ["월누적강사레슨", "mtdInstructorLessonSessions"])),
    전월동일일강사레슨: numberValue(firstValue(row, ["전월동일일강사레슨", "prevMonthSameDayInstructorLessonSessions"])),
    월누적그룹예약률: numberValue(firstValue(row, ["월누적그룹예약률", "mtdGroupReservationRate"])),
    전월동일일그룹예약률: numberValue(firstValue(row, ["전월동일일그룹예약률", "prevMonthSameDayGroupReservationRate"])),
    월누적그룹출석률: numberValue(firstValue(row, ["월누적그룹출석률", "mtdGroupAttendanceRate"])),
    전월동일일그룹출석률: numberValue(firstValue(row, ["전월동일일그룹출석률", "prevMonthSameDayGroupAttendanceRate"])),
  };
}
  ...
  return {
    ...
    매출일일누적: (input.매출일일누적 || [])
      .map((row) => ({
        ...
        월누적세전총액: numberValue(row.월누적세전총액),
        전월동일일세전총액: numberValue(row.전월동일일세전총액),
        월누적수업마진률: numberValue(row.월누적수업마진률),
        전월동일일수업마진률: numberValue(row.전월동일일수업마진률),
        월누적그룹세션: numberValue(row.월누적그룹세션),
        전월동일일그룹세션: numberValue(row.전월동일일그룹세션),
        월누적프라이빗: numberValue(row.월누적프라이빗),
        전월동일일프라이빗: numberValue(row.전월동일일프라이빗),
        월누적강사레슨: numberValue(row.월누적강사레슨),
        전월동일일강사레슨: numberValue(row.전월동일일강사레슨),
        월누적그룹예약률: numberValue(row.월누적그룹예약률),
        전월동일일그룹예약률: numberValue(row.전월동일일그룹예약률),
        월누적그룹출석률: numberValue(row.월누적그룹출석률),
        전월동일일그룹출석률: numberValue(row.전월동일일그룹출석률),
      }))
