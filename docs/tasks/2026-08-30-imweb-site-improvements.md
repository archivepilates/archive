...
- 영상구매 기구 메뉴의 첫 프레임 중앙 정렬과 위치 고정
...
- 영상구매 기구 메뉴는 링크·라벨을 처음부터 중앙 정렬하고, 한글·영문 전환 시 위치 이동 없이 투명도만 변경
- 관리자 저장 알림과 새로고침 후 `2026-08-30e` Header 값의 완전 일치 확인
- 1440px 호버 전환 0·60·140·320ms에서 링크·라벨·한글·영문 중심점 `717.67px` 유지
- 1024px 호버 전환에서 링크·라벨·한글·영문 중심점 `506.57px` 유지, 두 언어 레이어 `transform: none`
- 320px, 390px, 768px 모바일·태블릿 화면의 가로 넘침 0, 데스크톱 기구 메뉴 규칙 미노출
