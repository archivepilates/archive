...
- 신규 이메일 가입 1건당 초대자와 초대받은 친구에게 각각 3,000원. 초대자별 `Asia/Seoul` 달력 월 최대 30,000원·10건, 친구는 회원당 최초 1회입니다.
- 구매·휴대폰 인증은 요구하지 않습니다. 초대자 월 한도를 넘겨도 신규 친구의 최초 혜택은 유지합니다. 월 30,000원은 행사 전체 지출 한도가 아닙니다.
- 친구 적립은 별도 `inviteeStartsAt` 이후 가입부터 시작하며 `inviteeTested=true` 승인 설정을 추가로 요구합니다. 기존 초대자 시작 시각·원장·공급사 사유는 바꾸지 않습니다.
...
- 같은 파일 안에 초대자·친구의 원장을 분리하고 공급사 적립 사유도 역할별로 분리합니다. 각 역할은 한 번만 지급할 수 있습니다. 한쪽만 성공해도 성공한 쪽을 재지급하지 않으며, 실패·월 경계의 미완료 건은 운영자 확인 대상으로 남깁니다.
- 아래 확인된 결과는 최초 초대자 전용 버전의 이력입니다. 양쪽 적립 변경의 검증·배포 결과는 `docs/tasks/2026-09-15-imweb-referral-both-rewards.md`에 별도로 기록합니다.
