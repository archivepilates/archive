...
- `resolved`는 실제 재등록 완료, `excluded`는 회원이 재등록 의사가 없다고 밝힌 경우로 구분합니다.
- 두 상태 모두 현재 관리 목록에서는 숨기되 수강권 주기별 상담 이력은 보존합니다.
...
- 수강권 ID가 없거나 같은 상품명으로 재등록한 경우에도 `activeTickets`의 별도 보유 행은 후속 수강권으로 인식합니다.
- 보상 쿠폰과 상품권은 재등록 대상 수강권이나 재등록 후속권으로 판정하지 않습니다.
