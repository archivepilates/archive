...
- 횟수권 사용횟수는 운영자가 덮어쓸 수 없다. 기간권은 StudioMate 정지기간정보에서 총 계약 주수와 홀딩 제외 실제 사용 주수를 확인하고 근거를 남겨야 하며, 확인할 수 없으면 계산을 중단한다.
- 홀딩이나 기간 연장은 StudioMate 원천에 먼저 반영한다. CORE는 시작일·만료일·남은 기간을 표시하지만, 종료된 홀딩 이력이 활성 수강권 원천에 남지 않을 수 있어 운영자가 StudioMate 정지기간정보로 실제 사용 주수와 근거를 확인한다.
...
- 이번 개선 구현: 이름 후보 선택, canonical memberId 검증, 횟수권 잔여횟수 자동 산정, 기간권 원천 보조 표시, 횟수 수동 덮어쓰기 차단 완료
