...
- 라이브 검증: 격리 테스트 계정과 테스트 수강권으로 결제금액·위약금·사용 공제·예상 환불 계산을 확인
- 이폼싸인 실발송: 운영 프로필에서 테스트 수신자에게 SMS 단건 발송 완료. Firestore 작업 `done`, 케이스 `agreement_sent`, 수신 확인 기록 완료
- 실제 결제 환불, StudioMate 문자, 수강권 변경은 실행하지 않음
- v9 필드·수신자·문서함 증거 보강은 운영 승격과 라이브 검증 완료
