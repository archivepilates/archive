...
- 기존 수강권 기간/횟수/만료 안내 템플릿 3건은 새 v3 템플릿으로 대체하기 전까지 신규 자동화에 연결하지 않는다.

검수중 템플릿:

| 목적 | Template ID | 현재 상태 | 변수 |
|---|---|---|---|
| 그룹 기간권 잔여기간 안내 v3 | `KA01TP260514145047261araXgWLVFRs` | `INSPECTING` | `#{이름}`, `#{남은일수}`, `#{수강권명}` |
| 그룹 횟수권 잔여횟수 안내 v3 | `KA01TP260514145047393VpTbcCZKkCV` | `INSPECTING` | `#{이름}`, `#{잔여횟수}`, `#{수강권명}`, `#{만료일}` |
| 프라이빗 횟수권 잔여횟수 안내 v1 | `KA01TP260514152235608d9icGOBotnV` | `INSPECTING` | `#{이름}`, `#{잔여횟수}`, `#{수강권명}`, `#{만료일}` |
| 프라이빗 사전설문 안내 v1 | `KA01TP260514153632171uiWXYoeiOLS` | `INSPECTING` | `#{이름}` |

위 4건은 검수중이므로 `APPROVED` 상태가 되기 전까지 실제 발송에 연결하지 않는다. 그룹 기간권/횟수권에는 이용안내/홀딩규정 버튼을 붙였고, 프라이빗 사전설문에는 Google Form 버튼을 붙였다. 프라이빗 기간권 템플릿은 운영 빈도가 낮아 자동 연결 보류로 둔다.
