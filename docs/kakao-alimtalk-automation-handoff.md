...
| 프라이빗 사전설문 안내 v1 | `KA01TP260514153632171uiWXYoeiOLS` | `APPROVED` | `#{이름}` |

그룹 기간권/횟수권에는 이용안내/홀딩규정 버튼을 붙였고, 프라이빗 사전설문에는 Google Form 버튼을 붙였다. `long_absence`는 승인된 전용 템플릿이 생기기 전까지 알림톡 자동 후보를 만들지 않는다.
