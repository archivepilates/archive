...
사용자가 확인한 현재 라이브 템플릿 중 자동 발송에 바로 연결 가능한 템플릿은 승인 상태인 항목만입니다.
...
- 기존 수강권 기간/횟수/만료 안내 구 템플릿 3건은 신규 자동화에 연결하지 않는다.

현재 운영 가능 템플릿:

| 목적 | Template ID | 현재 상태 | 변수 |
|---|---|---|---|
| 그룹 기간권 잔여기간 안내 v3 | `KA01TP260514145047261araXgWLVFRs` | `APPROVED` | `#{이름}`, `#{남은일수}`, `#{수강권명}` |
| 그룹 횟수권 잔여횟수 안내 v3 | `KA01TP260514145047393VpTbcCZKkCV` | `APPROVED` | `#{이름}`, `#{잔여횟수}`, `#{수강권명}`, `#{만료일}` |
| 프라이빗 횟수권 잔여횟수 안내 v1 | `KA01TP260514152235608d9icGOBotnV` | `APPROVED` | `#{이름}`, `#{잔여횟수}`, `#{수강권명}`, `#{만료일}` |
| 프라이빗 기간권 잔여기간 안내 v1 | `KA01TP260514153314927WH270IppWQS` | `APPROVED` | `#{이름}`, `#{남은일수}`, `#{수강권명}` |
| 프라이빗 사전설문 안내 v2 | `KA01TP260729144645970fv13He8mfsK` | `APPROVED` | `#{이름}`, `#{링크ID}` |
| 강사용 프라이빗 차트 작성 안내 v3 | `KA01TP260729144657202OV26yAD15wR` | `APPROVED` | `#{강사명}`, `#{회원명}`, `#{회차}`, `#{수업일시}`, 링크 ID 3종 |
| 회원용 프라이빗 수업 리포트 안내 v1 | `KA01TP260528081225871Fr92FW901Vo` | `APPROVED` | `#{회원명}`, `#{리포트링크ID}` |

그룹 기간권/횟수권에는 이용안내/홀딩규정 버튼을 붙인다. 프라이빗 사전설문과 강사 기록은 ARCHIVE PILATES 자체 화면으로 연결하며, Notion은 회차별 차트 표시·열람용으로만 사용한다. `long_absence`는 승인된 전용 템플릿이 생기기 전까지 알림톡 자동 후보를 만들지 않는다.

2026-07-31에 프라이빗 사전설문 v1 `KA01TP260514153632171uiWXYoeiOLS`와 강사용 차트 v2 `KA01TP260527182741301uIuSTL01YQ1`를 SOLAPI에서 삭제했다. 과거 발송 기록 식별 외에는 참조하지 않는다.

템플릿별 대상자 선정과 변수 매핑은 `docs/solapi-template-data-operating-rules.md`를 기준으로 한다.
...
- 사전설문 작성: `https://in.archivepilates.com/s/#{링크ID}/`
...
v3는 `APPROVED` 상태이므로 신규회원 후보에 연결할 수 있다. 초기 운영에서는 운영자 확인 후 발송한다.
...
이벤트 수강권 홍보, 누적회원 대상 마케팅, 자유형 단체공지 등은 알림톡보다 브랜드 메시지 또는 채널 메시지로 분리하는 것이 안전하다.

수강료 링크 운영 기준:

- 수강료 노션 링크는 알림톡 본문이나 버튼에 넣지 않는다.
- 카카오채널 메뉴 버튼에 `수강료 안내`로 상시 노출한다.
- 알림톡을 받은 회원도 채널 화면에 진입하면 메뉴 버튼을 볼 수 있으므로, 가격/재등록 성격의 링크는 채널 메뉴로 우회한다.
- 알림톡 문구는 "필요하시면 카카오채널로 문의해 주세요" 수준으로 유지한다.
