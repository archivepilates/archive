...
- 운영자 화면과 회원카드에는 알림톡 상태 태그/칩을 표시하지 않는다. 후보 패널, 발송 이력, Google Drive 로그 문서, 메일 보고에서만 확인한다. 알림톡 보고/승인/미제출 알림 메일 제목은 `[알림톡][상태] 핵심내용` 형식을 사용하고, Gmail에는 `알림톡 보고` 라벨과 `자동화 성공`, `자동화 실패`, `자동화 긴급`, `자동화 확인필요` 중 하나를 함께 적용한다. 알림톡 후보/발송 실패가 있으면 `실패`, 발송 실패 없이 운영 확인 항목만 있으면 `확인필요`로 분류한다.
...
| 장기 미방문 수업안내 v1 | `KA01TP260524083643752cySb9BoDOjN` | 활성 수업 수강권 보유, 회원목록 엑셀 `최근출석일` 또는 예약 출석 완료일 중 더 최신값 기준 10일 이상, 보호 스텝/강사 연락처 제외, 정지/중지/홀딩 제외, 1회권/강사레슨 수강권 보유 제외, 발송 기준일 당일 또는 이후 예정 예약 제외. 같은 마지막 출석일 기준 1회만 발송. 2026-05-26 기준 감사 완료 전까지 자동 발송 임시 제외 |

구 수강권 기간/횟수/장기미방문 템플릿 코드는 신규 후보 생성에 연결하지 않는다. `long_absence`는 승인된 전용 템플릿과 연결하며 매일 11:30 자동 발송 대상에 포함한다.
...
- `dedupeKey` 확인 기간은 템플릿별로 다르게 둔다. 예약 오픈 안내는 6일, 수강권 기간/횟수 안내는 30일, 장기 미방문 안내는 같은 회원의 같은 마지막 출석일 기준 영구 1회, 신규회원 웰컴과 프라이빗 사전설문은 영구 1회로 차단한다.
