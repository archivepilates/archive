...
- 운영자는 후보 명단을 복사해 검토한 뒤 `일괄 승인`으로 승인 가능한 후보를 `queued` 상태로 넘긴다.
- 서버 워커 `scheduledProcessAlimtalkQueue`가 5분마다 `queued` 후보를 확인해 SOLAPI로 발송하고, `alimtalkSends/{candidateId}`에 결과를 남긴다.
