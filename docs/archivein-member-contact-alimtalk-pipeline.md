...
구 수강권 기간/횟수/장기미방문 템플릿 코드는 신규 후보 생성에 연결하지 않는다. `long_absence`는 승인된 전용 템플릿이 생기기 전까지 알림톡 자동 후보를 만들지 않는다.
...
- `dedupeKey` 확인 기간은 템플릿별로 다르게 둔다. 예약 오픈 안내는 6일, 수강권 기간/횟수 안내는 30일, 신규회원 웰컴과 프라이빗 사전설문은 영구 1회로 차단한다.
