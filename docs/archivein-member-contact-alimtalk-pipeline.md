...
| 장기 미방문 수업안내 v1 | `KA01TP260524083643752cySb9BoDOjN` | 활성 수업 수강권 보유, 회원목록 엑셀 `최근출석일` 또는 예약 출석 완료일 중 더 최신값 기준 10일 이상, 보호 스텝/강사 연락처 제외, 정지/중지/홀딩 제외, 1회권/강사레슨 수강권 보유 제외, 발송 기준일 당일 또는 이후 예정 예약 제외. 같은 마지막 출석일 기준 1회만 발송. 현재 `APPROVED`/`active`라 매일 11:30 자동 발송 대상 |
| 강사레슨 수업자료 안내 v1 | `KA01TP260521120040094XcMvYgFTryj` | 발송 기준일 다음 날 강사레슨 예약, 수업명 영문 주제명 기반 `영문주제-수업날짜6자리` 관리번호 자동 생성, 같은 회원/수업일/관리번호 1회만 발송 |

구 수강권 기간/횟수/장기미방문 템플릿 코드는 신규 후보 생성에 연결하지 않는다.
