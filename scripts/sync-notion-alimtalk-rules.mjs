...
function buildBlocks() {
  ...
  return [
    ...
    bullet("회원용_장기 미방문 수업안내 v1: Template ID `KA01TP260524083643752cySb9BoDOjN`, 현재 `APPROVED`/`active`. 매일 11:30 자동 발송 대상이다."),
    bullet("강사용_프라이빗 사전설문 제출 안내 v1"),
    bullet("강사용_그룹 사전확인 제출 안내 v1"),
    bullet("강사용_프라이빗 차트 작성 안내 v2: Template ID `KA01TP260527182741301uIuSTL01YQ1`, 현재 `PENDING`. 수업 전 계획, 수업 후 기록, 사진·영상 업로드 버튼 3개 구성이다."),
    bullet("강사레슨_수업자료 안내 v1: 아카이브강사레슨 채널, Template ID `KA01TP260521120040094XcMvYgFTryj`, 현재 `APPROVED`/`active`. 수업 하루 전 수업자료/방문안내 버튼 2개로 자동 발송한다."),
    heading(2, "자동화 규칙"),
    ...
    bullet("강사레슨 수업자료 안내는 수업 하루 전 발송한다. 강사레슨 예약의 수업명에서 영문 주제명을 추출하고 `영문주제-수업날짜6자리` 관리번호를 자동 생성한다. 버튼은 `수업자료 보기`와 `방문안내 보기` 2개를 사용한다."),
    bullet("강사용 프라이빗 차트 작성 안내 v2는 수업 하루 전 18:00 이후 담당 강사에게 보낸다. SOLAPI 버튼 URL은 전체 URL 변수가 아니라 `https://in.archivepilates.com/s/#{링크ID}/` 형태의 짧은 링크 ID 변수를 사용한다."),
