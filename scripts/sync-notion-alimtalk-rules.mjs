...
function buildBlocks() {
  ...
  return [
    ...
    bullet("회원용_신규회원 웰컴 안내 v4: Template ID `KA01TP2606010514226407Ml4St5Thkh`, 삭제되어 사용 금지. 신규 후보/발송 연결 금지."),
    bullet("회원용_신규회원 웰컴 안내 v5: Template ID `KA01TP260602101939427lPhGyuDLvFM`, 현재 `APPROVED`. 현장 웰컴 즉시발송용이며 회원가입서 작성 버튼은 `https://in.archivepilates.com/s/#{링크ID}/` 짧은 링크를 사용한다."),
    ...
    heading(2, "자동화 규칙"),
    bullet("SOLAPI 템플릿 승인 상태는 매일 10:00, 11:30 전체 알림톡 발송 직전, 월요일 12:30 예약 안내 발송 직전에 자동 동기화한다."),
    ...
    bullet("현장 웰컴 즉시발송은 `onsiteWelcomeRequests`가 `lookup_ready`가 된 뒤 직원이 웰컴 페이지의 `웰컴 알림톡 전송` 버튼을 클릭할 때만 발송한다. 신규회원 웰컴 v5 템플릿 코드/승인 전에는 발송하지 않고 lookup_ready 상태로 대기한다."),
    ...
    bullet("수업예약오픈안내는 월요일 12:30 KST 전용 루틴에서만 후보를 만든다. 매일 11:30 전체 알림톡 루틴과 분리하고, 예약 오픈 주간에 유효한 그룹/혼합 수강권 보유 회원만 대상이다."),
