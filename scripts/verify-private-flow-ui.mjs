...
const server = http.createServer(async (request, response) => {
  try {
    ...
    if (url.pathname === "/archivein/api/privateChart") {
      ...
      return json(response, 200, privateChartPayload(mode));
    }
  ...
  for (const viewport of [
    ...
    for (const mode of ["pre", "post"]) {
      ...
      await chartPage.goto(
        `${origin}/archivein/private-chart/?r=plc-ui-test&t=abcdefabcdefabcdefabcdefabcdefab&mode=${mode}`,
        { waitUntil: "networkidle" },
      );
      await chartPage.locator("#form").waitFor({ state: "visible" });
      await assertNoHorizontalOverflow(chartPage, `staff ${mode} ${viewport.width}px`);
      if (mode === "post") {
        await assertVisibleText(chartPage, "수업 사진·영상");
        await chartPage.locator("#mediaFiles").waitFor({ state: "attached" });
      }
      results.push({ surface: `staff-${mode}`, width: viewport.width, overflow: false });
      await chartPage.close();
    }
  }

  await verifyMemberSurveySubmission();
  await verifyStaffPreSubmission();
  await verifyStaffPostSubmission();

  assert.equal(submissions.filter((item) => item.kind === "member-intake").length, 1);
  assert.equal(submissions.filter((item) => item.kind === "staff-pre").length, 1);
    ...
    JSON.stringify(
      {
        ...
        submittedFlows: ["member-intake", "staff-pre", "staff-post"],
        checked: [
          "native member survey completion state",
          "staff pre-plan submission",
          "staff post-record submission",
...
async function verifyStaffPreSubmission() {
  const page = await browser.newPage({ viewport: { width: 390, height: 844 } });
  await page.goto(
    `${origin}/archivein/private-chart/?r=plc-ui-test&t=abcdefabcdefabcdefabcdefabcdefab&mode=pre`,
    { waitUntil: "networkidle" },
  );
  await page.locator('input[name="goals"][value="코어 안정성"]').check();
  await page.locator('input[name="focusAreas"][value="어깨"]').check();
  await page.getByRole("button", { name: "수업 전 계획 제출" }).click();
  await assertVisibleText(page, "수업 전 계획을 저장했습니다.");
  await page.close();
}
...
async function verifyStaffPostSubmission() {
  ...
  await page.locator('input[name="attendance"][value="출석"]').check();
  await page.locator('input[name="condition"][value="보통"]').check();
  await page.locator('input[name="changes"][value="기타"]').check();
  await page.locator('textarea[name="changesOther"]').fill("견갑 움직임을 편안하게 연결함");
  await page.locator('textarea[name="summaryKeywords"]').fill("흉곽 움직임, 호흡 연결");
  await page.locator('textarea[name="nextDirectionKeywords"]').fill("고관절 안정화, 코어 연결");
  await page.locator('textarea[name="homework"]').fill("자기 전 3분간 흉곽 호흡");
  await page.getByRole("button", { name: "수업 후 기록 제출" }).click();
  await assertVisibleText(page, "수업 후 기록을 저장했습니다.");
...
function privateChartPayload(mode) {
  return {
    ok: true,
    mode,
    memberName: "테스트 회원",
    ...
    lessonTime: "2026. 7. 30. 오전 10:00",
    staffName: "테스트 강사",
    preStatus: mode === "post" ? "submitted" : "pending",
    ...
    previousReport: {
      ...
      url: "https://in.archivepilates.com/s/test-report",
    },
    media: {
      files: [],
    ...
    report:
      mode === "post" && postSubmitted
        ? {
            ...
            canEdit: true,
            sent: false,
            url: "https://in.archivepilates.com/api/privateLessonReport?recordId=test",
            canonicalUrl: "https://in.archivepilates.com/api/privateLessonReport?recordId=test",
            summary:
