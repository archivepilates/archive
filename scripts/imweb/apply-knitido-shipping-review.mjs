...
const MARKER = "data-archive-pilates-knitido-shipping-review";
const INSTALLER = fs
  .readFileSync(path.join(ROOT, "scripts/imweb/install-knitido-shipping-review.html"), "utf8")
  .trim();
...
const liveKnitidoResponse = await fetch(
  "https://archivepilates.imweb.me/16?ap_shop=knitido",
  { redirect: "follow" }
);
if (!liveKnitidoResponse.ok) {
  throw new Error(`Knitido page preflight returned HTTP ${liveKnitidoResponse.status}`);
}
const liveKnitidoHtml = await liveKnitidoResponse.text();
if (!liveKnitidoHtml.includes('data-archive-pilates-knitido-brand-page="2026-07-19a"')) {
  throw new Error("live Knitido page is missing the active brand renderer");
}

const before = String(rows[0].scriptContent || "");
if (!before.includes('data-archive-pilates-my-classroom-v2="2026-07-28d"')) {
  throw new Error("current header is missing the active My Classroom loader");
}
...
const backupDirectory = fs.mkdtempSync(
  path.join(os.tmpdir(), "archive-imweb-knitido-shipping-review-")
);
