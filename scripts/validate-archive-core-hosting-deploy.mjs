...
const required = [
  ...
  },
  {
    ...
    markers: [
      ...
      "hiddenSections",
      'data-section="studiomate-member-registration"',
      "member-registration/",
  ...
  {
    file: "core/member-registration/index.html",
    label: "member registration progress dashboard",
    markers: [
      "data-member-registration-dashboard",
      "회원등록",
      "https://arcpilates.studiomate.kr/users/create",
      "memberRegistrationRecentCount",
      "memberRegistrationActiveCount",
      "memberRegistrationReviewCount",
      "memberRegistrationCompletedCount",
  ...
  },
  {
    ...
    label: "manager-only CORE operational reads",
    markers: [
      "match /staffEvaluationSubmissions/{submissionId}",
      "match /staffHrCards/{staffId}",
      "match /quizResults/{resultId}",
      "match /studiomateMembershipContracts/{contractId}",
    ...
    patterns: [
      ...
      {
        label: "manager-only StudioMate membership contract read block",
        pattern:
          /match \/studiomateMembershipContracts\/\{contractId\}\s*\{\s*allow read: if isManager\(\) && sameStudioClaim\(resource\.data\.studioId\);\s*allow write: if false;\s*\}/m,
      },
      {
  ...
  },
  {
    ...
    label: "member video watch analytics dashboard",
    markers: [
      "회원별 영상 시청 현황",
  ...
  },
  {
    ...
    label: "member video watch privacy disclosure",
    markers: [
      "2026년 9월 1일",
      "회원 전용 영상 시청 분석",
      "가명 처리된 회원 식별값",
      "회원 전용 영상 상세 시청 이벤트: 수집일로부터 180일",
      "회원 전용 영상 시청 세션 요약: 수집일로부터 1년",
      "구매·권한·환불 여부를 자동 결정하거나 광고 및 자동 메시지 발송 대상을 정하는 데 사용하지 않습니다",
    ],
  },
  {
    file: "scripts/imweb-video-watch-tracker.js",
    label: "fail-open member video watch tracker",
    markers: [
  ...
  {
    file: "core/assets/imweb-video-watch-tracker-20260901.js",
    label: "deployable member video watch tracker asset",
    markers: [
      "archive-method-watch-",
